...
# Sub-theme: "GSIAS HUFS Lecture 07"                      #
...
# Set specific date
Text.source <- Text.source[which(Text.source$date >= as.Date("2019-10-17") & Text.source$date <= as.Date("2020-12-31")),]

# 1st text cleansing pick the specific contents
Text.source <- Text.source %>%
               filter(str_detect(Text.source$text, "Hezbollah"))
...
kwic(Mining.text.corpus, pattern = "Hezbollah", valuetype = "regex")
...
Mining.TXT.token <- tokens(Mining.text.corpus, what = "word",
                           ...
                           # tokens_compound(list(c("world", "bank"))) %>%
                           # tokens_compound(list(c("uhuru", "kenyatta"))) %>%
                           # tokens_compound(list(c("foreign", "aid")))
...
tmod_lda <- textmodel_lda(Mining.TXT.token.dfm, k = 3)
terms(tmod_lda, 5)
...
quant_dfm <- tokens(Mining.text.corpus, remove_punct = TRUE, remove_numbers = TRUE) %>%
